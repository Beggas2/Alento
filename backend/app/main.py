...
def get_dashboard(role: str = "profissional", db: Session = Depends(get_db)):
    ...
    if role == "paciente":
        ...
        if patient:
            professionals = (
                db.query(models.PatientProfessional)
                .filter(models.PatientProfessional.patient_id == patient.id)
                .all()
            )
            professional_info = [
                schemas.ProfessionalInfo(
                    ...
                    especialidade=link.professional.user.profile.especialidade
                    if link.professional.user.profile
                    else None,
                    codigo=link.professional.user.profile.codigo if link.professional.user.profile else None,
                    is_medico=bool(link.professional.user.profile.is_medico)
                    if link.professional.user.profile
                    else False,
                )
            ...
            recent_records = (
                db.query(models.DailyRecord)
                    .filter(models.DailyRecord.patient_id == patient.id)
                    .order_by(models.DailyRecord.created_at.desc())
                    .limit(5)
                    .all()
            )
            link_requests = (
                db.query(models.LinkRequest)
                    .filter(models.LinkRequest.patient_id == patient.id)
                    .order_by(models.LinkRequest.created_at.desc())
                    .limit(5)
                    .all()
            )
