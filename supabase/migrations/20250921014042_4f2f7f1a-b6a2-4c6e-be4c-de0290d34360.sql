...
SELECT 
  ...
  -- Latest mood score - mood_level enum values are 1-10
  (
    SELECT CAST(dr.humor AS text)::integer
  ...
  -- PHQ-9 score (latest questionnaire completion)
  (
    SELECT latest_scores.total_score
    FROM (
      SELECT 
        SUM(qr.response_value) as total_score,
        MAX(qr.answered_at) as latest_date
  ...
  -- GAD-7 score (latest questionnaire completion)
  (
    SELECT latest_scores.total_score
    FROM (
      SELECT 
